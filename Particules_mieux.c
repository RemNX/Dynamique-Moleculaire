...
double EM(Particule *p1, Particule *p2)
    {
     double rij = distance(p1, p2);
     double E=(pow(p1->vx,2)+pow(p1->vy,2))/2 + (pow(p2->vx,2)+pow(p2->vy,2))/2;
 ...
 */
    ...
    for (j=0;j<20000;j++)
    {
        ...
        //printf("%f\n",EM(&tab_particules[0],&tab_particules[1])); //verification energie totale
        fprintf (out, "%22.8g  %22.8g \n", distance(&tab_particules[0],&tab_particules[1]),EM(&tab_particules[0],&tab_particules[1]));
    }
    fclose (out);
    //sprintf (command, "gnuplot -p -e \"plot 'potentiel.txt' w l , \" ");
    sprintf(command, "gnuplot -p -e \"set title 'Simulation Potentiel Lennard Joens' font ',16'; set xlabel 'r/d'; set ylabel 'E/E0'; plot 'potentiel.txt' w l\"");
